...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("pointerdown", ensureFullscreenAndLandscape, {
      passive: true,
    });
    document.addEventListener("touchstart", ensureFullscreenAndLandscape, {
      passive: true,
    });
  ...
  function onOrientationChange() {
    if (!isPortrait()) {
      ...
      if (
        blocker &&
        !document.documentElement.classList.contains("force-landscape-active")
      ) {
  ...
  /* ========================================
       6. Escalado dinámico para pantallas pequeñas
       El juego está diseñado para 1280px.
       En mobile landscape (~667-900px) escalamos
       el html para que todo quepa sin scroll.
       ======================================== */
  const DESIGN_WIDTH = 1280;

  function applyMobileScale() {
    // En rotación CSS el ancho efectivo es el alto del viewport portrait
    const isRotated = document.documentElement.classList.contains(
      "force-landscape-active",
    );
    const effectiveWidth = isRotated ? window.innerHeight : window.innerWidth;

    if (effectiveWidth < DESIGN_WIDTH) {
      const scale = parseFloat((effectiveWidth / DESIGN_WIDTH).toFixed(3));
  ...
  // Aplicar al cargar y en cada cambio de tamaño/orientación
  applyMobileScale();
  window.addEventListener("resize", applyMobileScale);
  window.addEventListener("orientationchange", () =>
    setTimeout(applyMobileScale, 350),
  );
})();