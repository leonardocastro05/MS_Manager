...
class DashboardController {
  ...
  updateUI() {
    ...
    document.getElementById("stat-wins").textContent =
      gameData.wins || stats.wins || 0;
    document.getElementById("stat-matches").textContent =
      gameData.racesCompleted || stats.matches || 0;
  ...
  async loadSocialOverview(silent = false) {
    try {
      ...
      if (!silent) {
        this.showFriendsFeedback(
          "No se pudo cargar la seccion de amigos",
          "error",
        );
      }
  ...
  renderSocialOverview() {
    ...
    if (incomingList) {
      incomingList.innerHTML = incomingRequests
        .map((request) => {
          const from = request.from || {};
          const displayName = this.escapeHtml(
            from.displayName || from.username || "Manager",
          );
    ...
    if (friendsList) {
      friendsList.innerHTML = friends
        .map((friend) => {
          ...
          const displayName = this.escapeHtml(
            friend.displayName || friend.username || "Manager",
          );
    ...
    if (raceInvitesList) {
      raceInvitesList.innerHTML = raceInvites
        .map((invite) => {
          const hostName = this.escapeHtml(
            invite.host?.displayName || invite.host?.username || "Manager",
          );
          const track = this.escapeHtml(
            (invite.trackId || "pista").toUpperCase(),
          );
  ...
  async sendFriendRequest() {
    ...
    const friendCode = String(input.value || "")
      .trim()
      .toUpperCase();
    if (!friendCode) {
      this.showFriendsFeedback(
        "Introduce un codigo de usuario valido",
        "error",
      );
      return;
    }
    ...
    try {
      if (action === "accept-request") {
        ...
        await this.apiRequest(
          `/social/friends/request/${encodeURIComponent(userId)}/accept`,
          {
            method: "POST",
          },
        );
        ...
        const userId = target.dataset.userId;
        await this.apiRequest(
          `/social/friends/request/${encodeURIComponent(userId)}/reject`,
          {
            method: "POST",
          },
        );
        ...
        const roomCode = target.dataset.roomCode;
        await this.apiRequest(
          `/social/quick-races/${encodeURIComponent(roomCode)}/reject-invite`,
          {
            method: "POST",
          },
        );
  ...
  bindEvents() {
    ...
    if (onlineCard) {
      onlineCard.addEventListener("click", (e) => {
        if (
          e.target.closest("#btn-online-competitive") ||
          e.target.closest("#btn-online-friendly")
        ) {
          return;
        }
    ...
    setTimeout(() => {
      switch (section) {
        ...
        case "friendly":
          ...
          break;
        case "shop":
          window.location.href = "/app/online.html#shop";
          break;
      }
    }, 100);
  }
