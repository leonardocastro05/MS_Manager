...
class LeagueController {
  ...
  async loadLeagueData() {
    try {
      const response = await fetch(
        `${this.API_URL}/online/leagues/${this.leagueId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
          },
      ...
      if (data.success && data.league) {
        ...
        this.leagueStandings = Array.isArray(data.standings)
          ? data.standings
          : this.leagueStandings;
        this.updateLeagueUI();
  ...
  async loadLeagueStandings() {
    try {
      const response = await fetch(
        `${this.API_URL}/online/leagues/${this.leagueId}/standings`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
          },
  ...
  async loadRaceCenter() {
    try {
      const response = await fetch(
        `${this.API_URL}/online/leagues/${this.leagueId}/race-center`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
          },
  ...
  async postOnlineRaceAction(endpointCandidates, payload = null) {
    ...
    return {
      ...
      data: lastFailure || {
        success: false,
        message: "No se encontro un endpoint compatible",
      },
      endpoint: null,
    };
  }
  ...
  async saveRaceStrategy() {
    const tyreCompound =
      document.getElementById("strategy-tyre")?.value || "medium";
    const pitLap = Number(
      document.getElementById("strategy-pitlap")?.value || 0,
    );
    const plannedLaps = Number(
      document.getElementById("strategy-laps")?.value || 20,
    );
    ...
    try {
      ...
      if (!data.success) {
        this.showToast(
          data.message || "No se pudo guardar la estrategia",
          "error",
        );
        return false;
      }
  ...
  async loadHQData() {
    try {
      const response = await fetch(
        `${this.API_URL}/online/leagues/${this.leagueId}/hq`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
          },
  ...
  async upgradeHQ(component) {
    ...
    const currentLevel = Number.isFinite(Number(comp?.level))
      ? Math.max(1, Number(comp.level))
      : 1;
    ...
    if (currentLevel >= accountLevel) {
      this.showToast(
        `Necesitas nivel ${currentLevel + 1} de cuenta para mejorar más`,
        "error",
      );
      return false;
    }
    ...
    try {
      const response = await fetch(
        `${this.API_URL}/online/leagues/${this.leagueId}/hq/upgrade`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (data.success) {
        ...
        this.updateHeaderStats();
        this.showToast(
          `¡${this.getComponentName(component)} mejorado a nivel ${comp.level}!`,
          "success",
        );
        return true;
      } else {
  ...
  async hirePilot(pilotIndex) {
    ...
    try {
      const response = await fetch(
        `${this.API_URL}/online/leagues/${this.leagueId}/pilot/hire`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async sellPilot() {
    ...
    try {
      const response = await fetch(
        `${this.API_URL}/online/leagues/${this.leagueId}/pilot/sell`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
      ...
      if (data.success) {
        ...
        this.updateHeaderStats();
        this.showToast(
          `Piloto vendido por ${this.formatMoney(sellPrice)}`,
          "success",
        );
      }
    } catch (error) {
      // Para desarrollo
      this.profile.gameData.budget =
        (this.profile?.gameData?.budget || 0) + sellPrice;
      this.currentPilot = null;
      this.updatePilotUI();
      this.updateHeaderStats();
      this.showToast(
  ...
  generateRandomPilot(marketLevel) {
    ...
    const pilot = {
      ...
      stats: {
        ...
        experience: Math.min(
          99,
          Math.floor(baseExperience + Math.random() * 10),
        ),
      },
    ...
    pilot.overall = Math.floor(
      (pilot.stats.speed + pilot.stats.control + pilot.stats.experience) / 3,
    );
  ...
  updateLeagueUI() {
    ...
    // Header
    document.getElementById("header-league-name").textContent =
      this.league.name;

    // Home section
    document.getElementById("league-name-display").textContent =
      this.league.name;
    document.getElementById("league-desc-display").textContent =
      this.league.description || "Sin descripción";
    document.getElementById("league-members").textContent =
      `${this.league.memberCount || this.league.members?.length || 1}/${this.league.settings?.maxMembers || 22}`;
    document.getElementById("league-country").textContent = this.getCountryName(
      this.league.country,
    );
    ...
    if (season) {
      ...
      document.getElementById("next-race").textContent =
        `${season.currentRace || 1} de ${season.totalRaces || 20}`;
    }
    ...
    if (deleteBtn && leaveBtn && this.profile) {
      ...
      const isCreatorDirect =
        creatorId && myUserId && creatorId.toString() === myUserId.toString();

      // Check if user has owner role in members
      const isOwnerRole =
        this.league.members &&
        this.league.members.some((m) => {
      ...
      console.log("League owner check:", {
        myUserId,
        creatorId,
        isCreatorDirect,
        isOwnerRole,
  ...
  updateHQUI() {
    ...
    Object.keys(this.hqComponents).forEach((key) => {
      ...
      const safeLevel = Number.isFinite(Number(comp.level))
        ? Math.max(1, Number(comp.level))
        : 1;
      ...
      if (costEl)
        costEl.textContent = this.formatMoney(this.calculateUpgradeCost(key));

      // Stats específicos
      if (key === "engine") {
        document.getElementById("engine-power").textContent =
          `+${safeLevel * 10}`;
        document.getElementById("engine-accel").textContent =
          `+${safeLevel * 5}`;
      } else if (key === "aero") {
        document.getElementById("aero-downforce").textContent =
          `+${safeLevel * 8}`;
        document.getElementById("aero-curves").textContent =
          `+${safeLevel * 6}`;
      } else if (key === "drs") {
        document.getElementById("drs-speed").textContent = `+${safeLevel * 12}`;
        document.getElementById("drs-efficiency").textContent =
          `+${safeLevel * 4}`;
      } else if (key === "chassis") {
        document.getElementById("chassis-durability").textContent =
          `+${safeLevel * 10}`;
        document.getElementById("chassis-resistance").textContent =
          `+${safeLevel * 5}`;
      } else if (key === "market") {
        const baseChance = 2;
        const bonusChance = safeLevel * 0.5;
        document.getElementById("market-chance").textContent =
          `${(baseChance + bonusChance).toFixed(1)}%`;
        document.getElementById("market-high-chance").textContent =
          `${(0.5 + safeLevel * 0.3).toFixed(1)}%`;
      }

      // Deshabilitar botón si alcanzó el límite
      const btn = document.querySelector(
        `.btn-upgrade-hq[data-component="${key}"]`,
      );
      if (btn) {
        if (safeLevel >= accountLevel) {
          ...
          btn.disabled = false;
          btn.innerHTML =
            '<span>Mejorar</span><span class="btn-icon">⬆️</span>';
        }
      }
    });
  ...
  renderPilotShop() {
    ...
    container.innerHTML = this.availablePilots
      .map((pilot, index) => this.renderPilotCard(pilot, false, index))
      .join("");
  ...
  getFallbackStandingsFromLeagueMembers() {
    const members = Array.isArray(this.league?.members)
      ? this.league.members
      : [];
  ...
  getEffectiveStandings() {
    if (
      Array.isArray(this.leagueStandings) &&
  ...
  updateMyStandingStats() {
    ...
    const myStanding = standings.find(
      (row) => (row.userId || "").toString() === myId,
    );

    const myPositionEl = document.getElementById("my-position");
    const myPointsEl = document.getElementById("my-points");
    if (myPositionEl)
      myPositionEl.textContent = myStanding ? `#${myStanding.position}` : "-";
    if (myPointsEl)
      myPointsEl.textContent = myStanding ? `${myStanding.points}` : "0";
  }
  ...
  renderMiniLeaderboard() {
    ...
    if (standings.length === 0) {
      container.innerHTML =
        '<div class="mini-lb-row"><span class="lb-name">Aún no hay managers en clasificación</span></div>';
      return;
    }
  ...
  renderFullStandingsTable() {
    ...
    if (standings.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="6">No hay datos de clasificación todavía.</td></tr>';
      return;
    }
  ...
  startPilotRefreshTimer() {
    ...
    if (savedTime) {
      const remaining = Math.max(
        0,
        parseInt(savedTime) - Math.floor(Date.now() / 1000),
      );
  ...
  showUpgradeHQModal(component) {
    ...
    if (safeLevel >= accountLevel) {
      ...
      document.getElementById("blocked-account-level").textContent =
        accountLevel;
  ...
  bindEvents() {
    ...
    document
      .getElementById("btn-go-hq")
      ?.addEventListener("click", () => this.navigateTo("headquarters"));
    document
      .getElementById("btn-go-pilots")
      ?.addEventListener("click", () => this.navigateTo("pilots"));
    document
      .getElementById("btn-go-shop")
      ?.addEventListener("click", () => this.navigateTo("shop"));
    document
      .getElementById("btn-go-races")
      ?.addEventListener("click", () => this.navigateTo("races"));
    document
      .getElementById("btn-view-standings")
      ?.addEventListener("click", () => this.navigateTo("standings"));
    ...
    // Confirm upgrade
    document
      .getElementById("confirm-upgrade-hq")
      ?.addEventListener("click", async () => {
        const component =
          document.getElementById("confirm-upgrade-hq").dataset.component;
        await this.upgradeHQ(component);
        this.closeModal("modal-upgrade-hq");
      });

    document
      .getElementById("cancel-upgrade-hq")
      ?.addEventListener("click", () => {
        this.closeModal("modal-upgrade-hq");
      });

    document
      .getElementById("close-upgrade-hq")
      ?.addEventListener("click", () => {
        this.closeModal("modal-upgrade-hq");
      });
    ...
    // Pilot hire
    document
      .getElementById("confirm-hire-pilot")
      ?.addEventListener("click", async () => {
        const index = parseInt(
          document.getElementById("confirm-hire-pilot").dataset.index,
        );
        await this.hirePilot(index);
      });

    document
      .getElementById("cancel-hire-pilot")
      ?.addEventListener("click", () => {
        this.closeModal("modal-hire-pilot");
      });

    document
      .getElementById("close-hire-pilot")
      ?.addEventListener("click", () => {
        this.closeModal("modal-hire-pilot");
      });
    ...
    // Instant refresh
    document
      .getElementById("instant-refresh")
      ?.addEventListener("click", async () => {
    ...
    // Skin purchase
    document
      .querySelectorAll(".btn-buy-skin:not([disabled])")
      .forEach((btn) => {
        btn.addEventListener("click", () => {
    ...
    document
      .getElementById("confirm-buy-skin")
      ?.addEventListener("click", () => {
    ...
    document
      .getElementById("cancel-buy-skin")
      ?.addEventListener("click", () => {
        this.closeModal("modal-buy-skin");
      });
    ...
    document
      .getElementById("btn-refresh-race-center")
      ?.addEventListener("click", async () => {
        await this.loadRaceCenter();
      });
    ...
    // Leave League button
    document
      .getElementById("btn-leave-league")
      ?.addEventListener("click", () => {
    ...
    document
      .getElementById("btn-delete-league")
      ?.addEventListener("click", () => {
  ...
  async leaveLeague() {
    try {
      const response = await fetch(
        `${this.API_URL}/online/leagues/${this.leagueId}/leave`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
  ...
  showBuySkinModal(skinId) {
    ...
    document.getElementById("purchase-skin-name").textContent =
      skinNames[skinId];
  ...
  async buyCoinPackage(packageId) {
    ...
    try {
      ...
      const response = await fetch(
        `${this.API_URL}/online/shop/buy-coins-simulated`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async loadDailyRacesStatus() {
    try {
      const response = await fetch(
        `${this.API_URL}/online/leagues/${this.leagueId}/daily-status`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
          },
  ...
  updateDailyRacesUI() {
    ...
    document.getElementById("races-max-daily").textContent =
      status.maxRacesPerDay;
  ...
  calculateXpForPosition(position) {
    const totalParticipants = Math.max(
      2,
      this.raceCenter?.participants?.length || 20,
    );
    const safePosition = Math.max(
      1,
      Math.min(totalParticipants, Number(position) || totalParticipants),
    );
    const step =
      (this.xpConfig.maxRaceXp - this.xpConfig.minRaceXp) /
      Math.max(1, totalParticipants - 1);
    return Math.max(
      this.xpConfig.minRaceXp,
      Math.round(this.xpConfig.maxRaceXp - (safePosition - 1) * step),
    );
  }
  ...
  async completeRace(position, raceData) {
    try {
      const response = await fetch(
        `${this.API_URL}/online/leagues/${this.leagueId}/race/complete`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      position === 1
        ...
        : position === 2
          ...
          : position === 3
            ...
            : "🏁";
    this.showToast(
      `${positionEmoji} P${position} - +${xp} XP ganados!`,
      "success",
    );
  }
  ...
  showLevelUpToast(newLevel) {
    this.showToast(
      `🎉 ¡SUBIDA DE NIVEL! Ahora eres nivel ${newLevel}`,
      "success",
    );
  }
  ...
  async loadChatMessages() {
    try {
      const response = await fetch(
        `${this.API_URL}/online/leagues/${this.leagueId}/chat?limit=50`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
          },
  ...
  async sendChatMessage() {
    ...
    try {
      const response = await fetch(
        `${this.API_URL}/online/leagues/${this.leagueId}/chat`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async prepareShopPurchase() {
    ...
    const remainingMs =
      this.purchaseCooldownMs - (Date.now() - this.lastPurchaseAt);
    if (remainingMs > 0) {
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, this.purchaseCooldownMs),
    );
    return true;
  }
  ...
  renderRaceCenter() {
    ...
    const participants = Array.isArray(this.raceCenter.participants)
      ? this.raceCenter.participants
      : [];
    const myId = (this.profile?._id || this.profile?.id || "").toString();
    const me =
      participants.find((participant) => participant.userId === myId) ||
      this.raceCenter.myParticipant ||
      null;
    ...
    if (seasonEl)
      seasonEl.textContent = this.league?.currentSeason?.number || 1;
    if (numberEl) numberEl.textContent = this.raceCenter.raceNumber || 1;
    if (totalEl)
      totalEl.textContent = this.league?.currentSeason?.totalRaces || 10;
    ...
    setText(
      "race-car-rating",
      Math.round(me?.carRating || this.calculateHQBonus() * 100),
    );
    ...
    if (strategyPit)
      strategyPit.value =
        myStrategy.pitLap || Math.max(3, Math.floor((track.laps || 20) / 2));
    if (strategyLaps)
      strategyLaps.value = myStrategy.plannedLaps || track.laps || 20;

    setText("current-track-name", track.name || "Circuito pendiente");
    setText(
      "current-track-location",
      `${track.flag || "🏁"} ${track.country || ""}`,
    );
    ...
    if (statusBadge) {
      if (!canQualifyNow) {
        statusBadge.textContent =
          schedule.message || "Carrera no disponible todavia";
      } else if (canStartRace) {
        statusBadge.textContent = `Clasificado P${me.gridPosition || "-"}`;
      } else {
        statusBadge.textContent = hasPilot
          ? "Esperando clasificación"
          : "Necesitas piloto para clasificar";
      }
    }
    ...
    if (qualifyingBody) {
      if (!participants.length) {
        qualifyingBody.innerHTML =
          '<tr><td colspan="6" class="empty-qualy">Todavia no hay managers en parrilla</td></tr>';
      } else {
        qualifyingBody.innerHTML = participants
          .map(
            (participant) => `
                    <tr>
  ...
  bindRaceEvents() {
    document.querySelectorAll(".race-mode-tab").forEach((tab) => {
      tab.addEventListener("click", () => {
        document
          .querySelectorAll(".race-mode-tab")
          .forEach((t) => t.classList.remove("active"));
        tab.classList.add("active");
      });
    });
    ...
    document
      .getElementById("btn-speed-1x")
      ?.addEventListener("click", () => this.setRaceSpeed(1));
    document
      .getElementById("btn-speed-2x")
      ?.addEventListener("click", () => this.setRaceSpeed(2));
    document
      .getElementById("btn-speed-5x")
      ?.addEventListener("click", () => this.setRaceSpeed(5));

    // Results actions
    document
      .getElementById("btn-close-results")
      ?.addEventListener("click", () => {
        document.getElementById("race-results-container").style.display =
          "none";
      });
  ...
  async loadCurrentTrack() {
    ...
    document.getElementById("current-track-location").textContent =
      `${track.flag} ${track.country}`;
    ...
    document.getElementById("track-turns").textContent =
      track.corners?.length || "-";
    document.getElementById("track-topspeed").textContent =
      `${track.characteristics?.topSpeed || 300} km/h`;
    document.getElementById("drs-zones-count").textContent =
      track.drsZones?.length || 0;
  ...
  async loadTrackSVG(svgPath, containerId) {
    ...
    try {
      // Si es imagen (jpg/png/webp), usar <img> directamente
      if (/\.(jpg|jpeg|png|webp)$/i.test(svgPath)) {
        container.innerHTML = `<img src="${svgPath}" style="width:100%;height:100%;object-fit:contain;" alt="Track preview">`;
        return;
      }

      // Para SVG: intentar fetch primero (funciona en servidor web)
      try {
      ...
      const svg =
        container.querySelector("svg") || container.querySelector("object");
  ...
  generateRaceWeather() {
    ...
    document.getElementById("race-weather-text").textContent =
  ...
  renderRaceCalendar() {
    ...
    container.innerHTML = this.raceCalendar
      .map((race, index) => {
    ...
    container
      .querySelectorAll(".calendar-race-card:not(.locked)")
      .forEach((card) => {
  ...
  async startQualifying() {
    try {
      if (
        this.raceCenter?.schedule &&
        this.raceCenter.schedule.canRaceNow === false
      ) {
        this.showToast(
          this.raceCenter.schedule.message ||
            "La carrera todavia no esta habilitada",
          "info",
        );
        return;
      }
      ...
      if (!data.success) {
        this.showToast(
          data.message || "No se pudo completar la clasificación",
          "error",
        );
      ...
      document.getElementById("race-status").textContent =
        `Clasificado P${data.position}`;
      document.getElementById("btn-start-race").disabled = false;
      this.renderRaceCenter();

      this.showToast(
        `Clasificación completada: P${data.position} (${data.lapTime})`,
        "success",
      );
    } catch (error) {
  ...
  generateRivalTimes(track, count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const time =
        baseTime * (1 - skill * 0.06) * (0.995 + Math.random() * 0.015);
  ...
  async startRace() {
    if (
      ...
      this.showToast(
        this.raceCenter.schedule.message ||
          "La carrera no esta habilitada en este momento",
        "info",
      );
      return;
    }
    ...
    if (
      !this.raceCenter ||
      !Array.isArray(this.raceCenter.participants) ||
      this.raceCenter.participants.length < 2
    ) {
      this.showToast(
        "No hay suficientes managers clasificados para correr",
        "error",
      );
      return;
    }
    ...
    const myId = (this.profile?._id || this.profile?.id || "").toString();
    const me = this.raceCenter.participants.find(
    ...
    const plannedLaps = Number(
      document.getElementById("strategy-laps")?.value ||
        this.raceCenter.track?.laps ||
        20,
    );
    const safeLaps = Number.isFinite(plannedLaps)
      ? Math.max(8, Math.min(40, plannedLaps))
      : 20;
    ...
    const participants = this.raceCenter.participants.map((participant) => {
      ...
      return {
        ...
        pilot: {
          ...
          speed:
            pilot.stats?.speed || Math.min(99, Math.max(50, pilotLevel + 25)),
          control:
            pilot.stats?.control || Math.min(99, Math.max(45, pilotLevel + 20)),
          experience:
            pilot.stats?.experience ||
            Math.min(99, Math.max(40, pilotLevel + 15)),
        },
        car: {
  ...
  updateRaceLiveUI(lapResult) {
    ...
    if (lapResult.fastestLap.pilot) {
      document.getElementById("fastest-lap-time").textContent =
        lapResult.fastestLap.time !== Infinity
          ? this.formatLapTime(lapResult.fastestLap.time)
          : "-";
      document.getElementById("fastest-lap-driver").textContent =
        lapResult.fastestLap.pilot?.name || "-";
    }

    // Clasificación en vivo
    const standingsContainer = document.getElementById("live-standings");
    standingsContainer.innerHTML = lapResult.positions
      .map(
        (p, i) => `
  ...
  async showRaceResults() {
    ...
    document.getElementById("results-track-name").textContent =
      results.track.shortName;
    document.getElementById("results-track-flag").textContent =
      results.track.flag;

    // Podium
    const podium = results.results.slice(0, 3);
    document.getElementById("podium-p1").textContent =
      podium[0]?.pilot.name || "-";
    document.getElementById("podium-p2").textContent =
      podium[1]?.pilot.name || "-";
    document.getElementById("podium-p3").textContent =
      podium[2]?.pilot.name || "-";
  ...
  togglePause() {
    ...
    document.getElementById("btn-pause-race").textContent = this.isPaused
      ? "▶️"
      : "⏸️";
  }
  ...
  getMockLeagueData() {
    return {
      ...
      name: "Liga de Prueba",
      description:
        "Esta es una liga de desarrollo para probar las funcionalidades",
      country: "ES",
      memberCount: 5,
      settings: {
        maxMembers: 22,
