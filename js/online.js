...
/**
 * ACTUALITZAR TOTA LA UI ONLINE (útil després de compres)
 */
function refreshOnlineUI() {
    updateOnlineHeaderBalances();
    loadOnlineUserInfo();
    
    // Actualitzar la secció actual visible
    const currentSection = document.querySelector('.online-section[style*="display: block"]');
    if (currentSection) {
        const sectionId = currentSection.id.replace('online-section-', '');
        if (sectionId === 'shop') loadOnlineShop();
        if (sectionId === 'sponsors') loadOnlineSponsors();
        if (sectionId === 'car') loadOnlineCarUpgrades();
        if (sectionId === 'xp') loadOnlineXPSystem();
        if (sectionId === 'overview') {
            loadOnlineUserInfo();
            if (typeof loadOnlineStats === 'function') loadOnlineStats();
        }
    }
}
